...
class ChatBot:
  ...
  def make_exit(self, user_message):
    for exit_com in exit_commands:
      if exit_com in user_message:
        print("Okay, have a great day ahead!")
        return True
  #define .chat() below:
  def chat(self):
    user_message = input("Hello, welcome to Mexican food arena, What can i help you with? ")
    while not self.make_exit(user_message):
      user_message = self.respond(user_message)
  #define .find_intent_match() below:
  def find_intent_match(self, responses, user_message):
   processed_user_msg = preprocess(user_message)
   bow_user_message = Counter(processed_user_msg)
   processed_res = [Counter(preprocess(response)) for response in responses]
   similarity_list = [compare_overlap(response, bow_user_message) for response in processed_res]
